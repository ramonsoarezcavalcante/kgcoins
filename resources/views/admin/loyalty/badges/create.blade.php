...
@section('title', 'Admin - Nova Conquista')

@section('content')
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-lg-12">
            <h1 class="h2">🎖️ Nova Conquista</h1>
        </div>
    </div>
                    ...
                    <form action="{{ route('admin.loyalty.badges.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('unlock_type') is-invalid @enderror" id="unlock_type" name="unlock_type" required onchange="toggleUnlockFields()">
                                    ...
                                    <option value="purchases_count" @selected(old('unlock_type') === 'purchases_count')>Número de Compras</option>
                            ...
                            <div id="purchases_count_field" class="mb-3" style="display: none;">
                                ...
                                <input type="number" class="form-control @error('unlock_count') is-invalid @enderror" id="unlock_count" name="unlock_count" value="{{ old('unlock_count', 1) }}" min="1">
                                @error('unlock_count')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div id="tier_reached_field" class="mb-3" style="display: none;">
                                ...
                                <select class="form-select @error('unlock_tier') is-invalid @enderror" id="unlock_tier" name="unlock_tier">
                                    ...
                                    @foreach($tiers as $tier)
                                        <option value="{{ $tier->id }}" @selected(old('unlock_tier') == $tier->id)>{{ $tier->name }}</option>
                                    @endforeach
                                </select>
                                @error('unlock_tier')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> Criar Conquista
                            </button>
